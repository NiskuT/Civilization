...
pass_by_reference(umlclass *cl)
{
    ...
    return (!is_const_stereo(st) && !is_enum_stereo(st) && !is_enum_class_stereo(st) && !is_union_stereo(st));
}
...
void print_include_stdlib(struct stdlib_includes *si, char *name)
{
    if (strlen(name) > 0)
    {
        if (!si->stdint && (strstr(name, "int8_t") || strstr(name, "uint8_t") || strstr(name, "int16_t") || strstr(name, "uint16_t") || strstr(name, "int32_t") || strstr(name, "uint32_t") || strstr(name, "int64_t") || strstr(name, "uint64_t")))
        {
